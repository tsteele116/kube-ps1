...
_kube_ps1_get_context_ns() {
  ...
  KUBE_CONTEXT="$(${KUBE_PS1_BINARY} config current-context 2>/dev/null)"
  if grep -q "aws" <<< "$KUBE_CONTEXT"; then
    KUBE_PS1_CONTEXT=$(echo $KUBE_CONTEXT|cut -d"/" -f2)
  else
    KUBE_PS1_CONTEXT=$KUBE_CONTEXT
  fi
